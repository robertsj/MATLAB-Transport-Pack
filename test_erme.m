...
clear
 inputrf = Input();
 put(inputrf, 'rf_db_name',        'two_group_3x3.h5');
 put(inputrf, 'rf_order_space',    0); % Eventually, these will be the
 put(inputrf, 'rf_order_azimuth',  0); % orders extracted from a DB
 put(inputrf, 'rf_order_polar',    0); 
 put(inputrf, 'rf_interp_method',  'linear'); 
% %put(db_input, 'rf_db_name', 'homog.h5');
 server = ResponseDB(inputrf);
 read_response(server);
save('test_erme_server.mat','server');
return
...
%solver = ERME_Newton(input, problem);
solver = ERME_Picard(input, problem);
tic, solve(solver); toc
disp(['number updates = ', num2str(server.number_updates)])
