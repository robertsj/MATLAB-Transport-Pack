%> @file  Sweep3D_mod.m
%> @brief Sweep3D_mod class definition.
% ==============================================================================
%> @brief Sweep over a 3D mesh.
%
...
% ==============================================================================
classdef Sweep3D_mod < SweepBase
    
    properties
        %> Place holder for a row of edge fluxes [num_x]
        d_psi_edge
        %> Place holder for planar edge fluxes [num_x*num_y]
        d_psi_plane
        %
        d_kernel = 1;
    end
    ...
    methods
        ...
        %>
        %> @return              Instance of the Sweep3D class.
        % ======================================================================
        function obj = Sweep3D_mod(input, mesh, mat, quadrature, boundary, ...
                equation)
            ...
            obj.d_psi_edge  = zeros(this.d_nx, 1);
            obj.d_psi_plane = zeros(this.d_nx*this.d_ny, 1);
        ...
        function phi = sweep(obj, s, g)
            ...
            for o = 1:4
                ...
                con_z = get_con_z(obj.d_equation, o);
                ...
                if obj.d_kernel == 1
                    ...
                    [phi, psi_v, psi_h] = ...
                        Sweep3Dkernel_opt2_mex(...
                        phi, psi_v, psi_h, obj.d_nx, yb, xb, sig, ...
                        con_x, con_y, s, wt, beta);
                    
                else
                    
                    % Sweep over all cells.
                    for k = zb(1):zb(3):zb(2)
    ...
    methods (Access = private)
        ...
        function bz = z_bounds(obj, o)
            if obj.d_quadrature.octant(o, 3) == 1
                bz = [1 number_cells_z(obj.d_mesh) 1];
            else
                bz = [number_cells_z(obj.d_mesh) 1 -1];
            end
        end
